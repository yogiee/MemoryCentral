...
export function start(port = 9980) {
  ...
  server.on('error', err => {
    ...
    // EADDRINUSE means another session already has the dashboard — skip silently
  });
  ...
  // Don't let the dashboard server keep the process alive on its own
  server.unref();
}
