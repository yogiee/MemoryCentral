...
_Last synced: 2026-05-22 05:36 UTC_

---

## ai-test

_synced: 2026-05-22T05:36:49Z | files: 8_
...
_synced: 2026-05-22T05:36:49Z | files: 3_
...
_synced: 2026-05-22T05:36:49Z | files: 5_
...
- **260521**: BugHerd #325 **FULLY DONE** — PR #403 updated. Wired `GoogleAdsTools::definitions()` into seeder (source of truth); migration changed to insert-only bootstrap (skips if seeder already populated rows, keyed on `server_definition_id + name`); hardcoded `'id'` values stripped from migration `toolRows()`; integrity test updated to 302 assertions (covers all 36 files + GoogleAdsTools). No open questions remain.
- **260521**: BugHerd triage — fresh API pull of all 23 active tickets. Key findings: #287 + #337 show as READY FOR REVIEW/QA but PRs already merged (Michael hasn't closed tickets); #103 (assistant templates) stale 10mo/7mo no plan; #58 IN PROGRESS 11mo stale; #18 oldest ticket 1yr+ unassigned; #326 (probe reliability) is only critical unstarted bug with no plan; #263–266 (skill MCP refactors) critical + unassigned + 3mo stale.
- **260518**: BugHerd #347/#348 — both largely fixed by Michael's PR #358 (merged May 10) before our session. Created PR #400 (`bugfix_render-plain-newlines_yogi`) for the one residual gap: `_renderUserPlain()` in `edward-streaming-ui.js` didn't convert `\n` → `<br>`, collapsing Shift+Enter line breaks in plain-text messages. One-liner fix + new test in `ChatInputRenderingTest.php`. Also applied PR #358's MEDIUMTEXT migration to `tenant_testtenant` to clear 3 pre-existing test failures. BugHerd #347 + #348 → READY FOR REVIEW/QA.
- **260518**: PR #350 UI fixes — swapped bell → `rocket-launch` icon on Edward toolbar button; fixed "Mark all read" button crashing into Flux flyout close (×) by moving it below the panel title. Also updated panel header icon from `bell` to `rocket-launch`. Pushed to `feature_edward-release-notes_yogi`.
...
_synced: 2026-05-22T05:36:49Z | files: 9_
...
- [Budget Context](project_budget.md) — base ¥1M JPY (~₹6.3L), stretch ¥1.5M JPY (~₹9L); monthly envelope ¥167K–250K; share houses ruled out, private monthly apartments only
- [Research Dashboard](reference_dashboard.md) — local web dashboard at port 8030; launch with start-dashboard.sh; charts auto-read from markdown files
- [Group Trip Profile](group_profile.md) — majority pure vegetarian; several members 100kg+, 5'0"–5'5"; western beds + chairs mandatory; private onsen; Green Car shinkansen; 10am starts
- [Next Session Priorities](project_next_priorities.md) — travel insurance, San'in deep dive, Shikoku deep dive, visa consulate inquiry (Session 14 onwards)
- [Server Behavior](feedback_server_behavior.md) — never kill/restart the dashboard server during visual tweaks; user refreshes manually

---

## LAiMA

_synced: 2026-05-22T05:36:49Z | files: 7_
...
_synced: 2026-05-22T05:36:49Z | files: 2_

# MemoryCentral Memory Index

- [Directory naming convention](feedback_directory_naming.md) — avoid hyphens in project dir names; Claude path encoding makes hyphenated dirs ambiguous
...
_synced: 2026-05-22T05:36:49Z | files: 4_
